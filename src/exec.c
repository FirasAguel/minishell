...
char	*handle_relative_path(const char *cmd)
{
	...
	if (access(cmd_escaped_str, F_OK) != 0) // No such file
		return (perror(cmd_escaped_str), free(cmd_escaped_str), NULL);
	else if (access(cmd_escaped_str, X_OK) != 0) // Not executable
